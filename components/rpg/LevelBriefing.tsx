...
export default function LevelBriefing({ levelId, onComplete }: LevelBriefingProps) {
    ...
    const getLines = (id: ChapterId): DialogueLine[] => {
        switch (id) {
            case 'chapter1-migration':
                return [
    ...
    // Handle empty lines case in useEffect to avoid setState during render
    useEffect(() => {
        if (lines.length === 0) {
            setShouldShow(false);
            onComplete();
